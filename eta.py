...
histMC, bins =  np.histogram(x, bins=np.arange(-3.,3.,0.06));
histL1, bins = np.histogram(z, bins=np.arange(-3.,3.,0.06));
...
ax1.plot(np.arange(-3.,2.94,0.06), po, 'r-', color='r')
ax1.set_xlabel('time (s)')
# Make the y-axis label and tick labels match the line color.
ax1.set_ylabel('Efficiency', color='r')

ax2 = ax1.twinx()
ax2.bar(bins[:-1], histMC, width=0.06, color='green', alpha=0.5)
plt.ylim([0,max(histMC)]+0.1*max(histMC))


ax3 = ax1.twinx()
ax3.bar(bins[:-1], histL1, width=0.06, color='blue', alpha=0.5)
